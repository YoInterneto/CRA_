...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;; PECL3 Conocimiento y razonamiento automatizado ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;;;;;;vectores

(define testvectores (lambda (m)
...
(define vectores (lambda (a)
                         (lambda (b)
                              ((par a) b))))


(define vector_prueba1 ((vectores dos) cuatro))

(define vector_prueba2 ((vectores uno) dos))

(define vector_prueba3 ((vectores dos) uno))

(define vector_prueba4 ((vectores uno) -tres))

;;;;;;;; condiconal if ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(define if (lambda (p x y) (p x y)))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;; Parte de los numeros enteros ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;


;;;;;; a) Reduccion a representante canónico;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

; Reduccion a representante canónico ----> (testenteros((reduccion cinco)tres))
(define reduccion (lambda (x)
                    (lambda (y)
                       ((restoent x) y)))) 
                                   


;;;;;; b) Aritmética de enteros ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(define resta_p (lambda (x)
                 (lambda (y)
                   (lambda (z)
                     ((reduccion ((restaent x) y))z)))))                 


; Producto de enteros ----> (testenteros(((prod_p cinco)tres)tres))
(define prod_p (lambda (x)
                 (lambda (y)
                   (lambda (z)
                     ((reduccion ((prodent x) y))z)))))





;;;;;;; c) Decisión de inversibilidad y calculo de inversa;;;;;;;;;;;;;;;;;;

; Decision de inversibilidad ----> ((inversible?_p cinco)tres)
(define inversible?_p (lambda (x)
                        (lambda (y)
                          ((esigualent((mcdent x) y))uno))))

; Calculo de la inversa del entero ----> (testenteros((inversa_ent cinco) nueve))
(define inversa_ent
    (lambda (a)
        (lambda (b)
            ((Y (lambda (f)
                (lambda (y)
                    (((neg((esigualent((mcdent a)b))uno))
                        (lambda (no_use)
                            cero
                        )
                        (lambda (no_use)
                          ((((esigualent (((prod_p y) a) b))uno)
                           (lambda (no_use1)
                             y)
                           (lambda (no_use1)
                             (f ((sument y) uno)))) cero)
...
; Suma de matrices ----> (testmatrices(((suma_matrices matriz_prueba1) matriz_prueba2) tres))
(define suma_matrices (lambda (x)
                        (lambda(y)
                          (lambda(z)
                            ((par ((par (((suma_p (primero (primero x)))(primero(primero y)))z)) (((suma_p (segundo (primero x)))(segundo(primero y)))z)))
                             ((par (((suma_p (primero (segundo x)))(primero(segundo y)))z)) (((suma_p (segundo (segundo x)))(segundo(segundo y)))z)))))))

; Resta de matrices ----> (testmatrices(((resta_matrices matriz_prueba1) matriz_prueba2) tres))
(define resta_matrices (lambda (x)
                        (lambda(y)
                          (lambda(z)
                            ((par ((par (((resta_p (primero (primero x)))(primero(primero y)))z)) (((resta_p (segundo (primero x)))(segundo(primero y)))z)))
                             ((par (((resta_p (primero (segundo x)))(primero(segundo y)))z)) (((resta_p (segundo (segundo x)))(segundo(segundo y)))z)))))))



; Producto de una matriz por otra matriz ----> (testmatrices(((prod_matrices matriz_prueba1) matriz_prueba2) tres))
(define prod_matrices (lambda (x)
                        (lambda(y)
                          (lambda(z)
                            ((par ((par (((suma_p ((prodent(primero (primero x)))(primero(primero y))))((prodent(segundo (primero x)))(primero(segundo y))))z))
                                   (((suma_p ((prodent(primero (primero x)))(segundo(primero y))))((prodent(segundo (primero x)))(segundo(segundo y))))z)))
                             ((par (((suma_p ((prodent(primero (segundo x)))(primero(primero y))))((prodent(segundo (segundo x)))(primero(segundo y))))z))
                                   (((suma_p ((prodent(primero (segundo x)))(segundo(primero y))))((prodent(segundo (segundo x)))(segundo(segundo y))))z)))))))

; Producto de una matriz por un número entero ----> (testmatrices(((prod_matriz_escalar matriz_prueba1) cinco) tres))
(define prod_matriz_escalar (lambda (x)
                        (lambda(y)
                          (lambda(z)
                            ((par ((par (((prod_p (primero (primero x)))y)z)) (((prod_p (segundo (primero x)))y)z)))
                             ((par (((prod_p (primero (segundo x)))y)z)) (((prod_p (segundo (segundo x)))y)z)))))))

; Producto de una matriz por un vector ----> (testvectores(((prod_matriz_vector matriz_prueba1) vector_prueba1) tres))
(define prod_matriz_vector (lambda (x)
                        (lambda(y)
                          (lambda(z)
                            ((par (((suma_p((prodent(primero y))(primero(primero x))))((prodent(segundo y))(segundo(primero x))))z))
                             (((suma_p((prodent(primero y))(primero(segundo x))))((prodent(segundo y))(segundo(segundo x))))z))))))

;;;;;;; b) Calculo del determinante de una matriz ;;;;;;;;;;;;;;;;;;;;;;;;;;;

; Calculo del determinante ----> (testenteros ((determinante_p matriz_prueba1)tres))
(define determinante_p (lambda (x)
                         (lambda (y)
                           (((resta_p ((prodent(primero (primero x)))(segundo(segundo x))))((prodent(segundo (primero x)))(primero(segundo x))))y))))


;;;;;;; c) Calculo y decision de la inversa, y cálculo del rango ;;;;;;;;;;;;
(define matriz_prueba9 ((((matriz cero) cero) -uno) cero))
; Decision de inversibilidad ----> ((inversa?_p matriz_prueba1)tres))
(define inversa?_p (lambda (x)
                     (lambda (y)
                       (noesceroent ((determinante_p x) y)))))


; Calculo de la inversa ----> (testmatrices ((inversa matriz_prueba1) tres))
(define inversa (lambda (x)
                  (lambda (y)
                    (((prod_matriz_escalar (calcular_matriz x)) ((inversa_ent ((determinante_p x)y))y)) y) )))
;Auxiliar que calcula la transpuesta y la adjunta a la vez
(define calcular_matriz (lambda (x)
                          ((par
                               ((par (segundo (segundo x)))
                                     ((prodent (segundo (primero x))) -uno)))
                               ((par ((prodent (primero (segundo x))) -uno))   
                                     (primero (primero x)))) ))

; Calculo del rango de una matriz ----> (testenteros ((rango_p matriz_prueba1) tres))
(define rango_p (lambda (x)
                  (lambda(y)
                     ((esceroent ((determinante_p x)y)) uno dos))))


;;;;;;; d) Calculo de potencias de matrices ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

; Calculo del cuadrado de una matriz ----> (testmatrices((cuadrado_matriz matriz_prueba1) tres))
(define cuadrado_matriz (lambda (x)
                          (lambda (y)
                          (((prod_matrices x)x)y))))



; Calculo de potencia de una matriz ----> (testmatrices(((potencia_matriz matriz_prueba1)trois) tres))
(define potencia_matriz ;;;el exponente se mete natural por que el metodo par? esta definida para naturales no paa enteros
  (lambda (m)
        (lambda (e)
          (lambda (mod)
            (((Y (lambda (f)
                     ...
                     (lambda(expo)
                      ((((esigualnat expo)un)  
                       ...
                       (lambda (no_use)
                            (((par? expo)
                              (lambda (no_use2)
                                   ((f (((prod_matrices matriz)matriz)mod)) ((cocientenat expo)deux))
                                )
                              (lambda (no_use2)
                                   (((prod_matrices matriz) ((f matriz) ((restanat expo)un)))mod)
                                ))zero)))zero)))))m)e)))))
